...
```json
[
  { "id": 42, "slug": "jane", "name": "Jane Smith", "avatar_url": "https://…" }
]
```
...
```html
<a class="p2026-mention"
   href="https://example.com/author/jane/"
   data-user-id="42"
   data-user-slug="jane">@jane</a>
```

The `data-user-id` attribute is what the frontend hovercard script uses to fetch rich profile data. The function avoids mutating HTML attribute values and tag contents by splitting on HTML tags and comments before applying the regex, so only bare text nodes are processed.
